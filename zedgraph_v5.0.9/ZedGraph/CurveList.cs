...
namespace ZedGraph
{
	...
	#region Properties
        ...
        // Kang Added
        // Only for CandleStick Render
        public int numBarsToFocus = 100;
        public int numBarsToEnd = 0;
		...
		public void GetRange( bool bIgnoreInitial, bool isBoundedRanges, GraphPane pane )
		{
			...
			foreach ( CurveItem curve in this )
			{
				if ( curve.IsVisible )
				{
                    ...
                    // 修正時間軸頭尾
                    xScale._rangeMax = tXMaxVal - numBarsToEnd;
                    xScale._rangeMin = tXMaxVal - numBarsToEnd - numBarsToFocus;
                    //Console.WriteLine("tXMaxVal " + tXMaxVal.ToString() + " : tXMinVal " + tXMinVal.ToString() );
				}
			}
